...
public class FourLights extends JPanel implements GLEventListener {
    ...
    private JCheckBox baseLight;  // Checked if the red light is on.
    private JCheckBox secondLight;  // Checked if the green light is on.
    private JCheckBox thirdLight;  // Checked if the blue light is on.
    ...
    /**
     * The constructor adds seven check boxes under the display, to control the options.
     */
    public FourLights() {
        ...
        baseLight = new JCheckBox("Base Light", true);
        thirdLight = new JCheckBox("Blue Light", true);
        secondLight = new JCheckBox("Green Light", true);
        ...
        baseLight.addActionListener(boxHandler);
        secondLight.addActionListener(boxHandler);
        thirdLight.addActionListener(boxHandler);
    ...
    private void lights(GL2 gl) {
        ...
        gl.glPushMatrix();
        gl.glRotated(5*frameNumber, 0, 1, 0);
        gl.glTranslated(9, .3*(frameNumber%50), 0);
        gl.glLightfv(GL2.GL_LIGHT2, GL2.GL_POSITION, zero, 0);
        glut.glutSolidSphere(0.5, 16, 8);
        gl.glPopMatrix();
        
        if (secondLight.isSelected()) {
            float green[] = {0, 0, 1F, 1 };
        ...
        gl.glPushMatrix();
        gl.glRotated((5*frameNumber+120), 0, 1, 0);
        gl.glTranslated(9, .3*(frameNumber%50), 0);
        gl.glLightfv(GL2.GL_LIGHT2, GL2.GL_POSITION, zero, 0);
        glut.glutSolidSphere(0.5, 16, 8);
        gl.glPopMatrix();
        
        if (thirdLight.isSelected()) {
            float blue[] = { 0, 0, 1F, 1 };
        ...
        gl.glPushMatrix();
        gl.glRotated((5*frameNumber+240), 0, 1, 0);
        gl.glTranslated(9, .3*(frameNumber%50), 0);
        gl.glLightfv(GL2.GL_LIGHT2, GL2.GL_POSITION, zero, 0);
        glut.glutSolidSphere(0.5, 16, 8);
        gl.glPopMatrix();
    ...
    public void display(GLAutoDrawable drawable) {    
        ...
        gl.glColor3d(0.7,0.7,0.7);  // sets diffuse and ambient color for plane

        gl.glMaterialfv(GL2.GL_FRONT_AND_BACK, GL2.GL_SPECULAR, new float[] {0.2F, 0.2F, 0.2F, 1 }, 0);
        gl.glRotatef(170, .1F, 1, .1F) ;
         gl.glScalef(7, 7, 7);

        gl.glColor3d(1,.1,.1);
        gl.glPushMatrix();
        gl.glPushMatrix();
        glut.glutSolidCylinder(.2F, 1F, 16, 16) ;
        gl.glRotatef(90, 0, 1, 0) ;
        gl.glTranslatef(-.3f, 0f, -.75F);
        glut.glutSolidCylinder(.1F, 1.5F, 8, 8) ;
        gl.glPopMatrix();
        
        gl.glPushMatrix();
        gl.glTranslatef(0f, .2f, .2F);
        drawSphere(.08, 16, 16, gl) ;
        gl.glPopMatrix();
        
        gl.glTranslatef(0f, 0f, .9F);
        gl.glRotatef(-70, 1, 0, 0) ;
        gl.glColor3d(1,.1,.1);
        glut.glutSolidCone(.08,.6,8,8) ;
        gl.glPopMatrix();
    }
    
     //modified from ColorCubeOfSpheres since it doesn't need checks for poor user input and is always run on the radius
    void drawSphere(double radius, int slices, int stacks,GL2 gl) {
        gl.glColor3d(1,1,1);
